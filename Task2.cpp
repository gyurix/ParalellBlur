...
unsigned char calcColor(unsigned char* in, int id) {
    unsigned int sum = 0;
    int x = id / 4 % 3840;
    int y = id / 4 / 3840;
    int minx = -x;
    ...
    for (int y = miny; y <= maxy; ++y) {
        for (int x = minx; x <= maxx; ++x) {
            int id2 = id + y * 3840 * 4 + x *4;
            sum += in[id2];
        }
    }
    return sum / ((maxy - miny + 1) * (maxx - minx + 1));
}
...
int main(int argc, char* argv[]) {
    ...
    unsigned char *y= (unsigned char*)malloc(LENGTH);        /* Receive buffer */
    ...
    if (me == 0) {    /* Process 0 does this */
        BMP in{ "in.bmp" };
        unsigned char* x = in.data.data();
   

        /* Scatter the array x to all proceses, place it in y */
        for (i = 1; i < np; i++) {
            MPI_Send(x, LENGTH, MPI_CHAR, i, datatag, MPI_COMM_WORLD);
        }

        for (int i = 0; i < LENGTH / np; ++i) {
            y[i] = calcColor(x, i);
        }

        for (i = 1; i < np; i++) {
            MPI_Recv(y + (i * LENGTH / np), LENGTH / np, MPI_CHAR, i, datatag, MPI_COMM_WORLD, &status);
        }
        memcpy(in.data.data(), y, LENGTH);
        char fn[10]= "outXX.bmp";
        fn[3] = np / 10 + 48;
        fn[4] = np % 10 + 48;
        in.write(fn);
    }
    else { /* all other processes do this */
        unsigned char* x = (unsigned char*)malloc(LENGTH);
        MPI_Recv(x, LENGTH, MPI_CHAR, root, datatag, MPI_COMM_WORLD, &status);
        int offset = LENGTH / np * me;
        for (int i = 0; i < LENGTH / np; ++i) {
            y[i] = calcColor(x, i+offset);
        }
        MPI_Send(y, LENGTH / np, MPI_CHAR, 0, datatag, MPI_COMM_WORLD);
